 ...
 */
 class ViewManager extends Smarty {
    ...
    public function __construct($config_array=null) {
		...
		$src_root_path = GTMVC_WEBAPP_ROOT.'/';
        $site_root_path = GTMVC_WEBAPP_ROOT.'/';
        $app_title = 'GTVMC';
        $cache_pages = false;
        $cache_lifetime = 0;
        $debug =  false;
        ...
        $this->Smarty();
        $this->template_dir = array(GTMVC_ROOT_PATH.'/_lib/view');
        $this->compile_dir = GTMVC_ROOT_PATH.'/data/compiled_view';
        $this->plugins_dir = array('plugins', GTMVC_ROOT_PATH.'_lib/view/plugins/');
        $this->cache_dir = GTMVC_ROOT_PATH.'/data/cache';
    ...
    public function fetch($template, $cache_key=null, $compile_id=null, $display=false) {
        $continue = false;
        if (is_writable(GTMVC_ROOT_PATH.'/')) {
			$ret = GTMVC_ROOT_PATH.' is writable';
