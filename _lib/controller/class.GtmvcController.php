 ...
 */
abstract class GtmvcController {
    /**
     * @var ViewManager
     */
    protected $view_mgr;
    ...
    */
    ...
    public function __construct($caller_file_path=null) {
        try {
			...
			$this->addToView('GTMVC_WEBAPP_ROOT', GTMVC_WEBAPP_ROOT);
	
        } catch (Exception $e) {
            Loader::definePathConstants();
            //echo 'sending this to Smarty:'.ADMIN_ROOT.'/data/';
            $cfg_array =  array(
            'site_root_path'=>GTMVC_ROOT_PATH,
            'source_root_path'=>GTMVC_ROOT_PATH,
            'datadir_path'=>GTMVC_ROOT_PATH.'/data/',
            'debug'=>false,
            'app_title_prefix'=>"",
            'cache_pages'=>false);
